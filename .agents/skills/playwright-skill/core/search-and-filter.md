...
test('filters products by a single category', async ({ page }) => {
  ...
  await expect(
    page
      ...
      .or(page.locator('[data-testid="active-filter"]').filter({ hasText: 'Electronics' })),
  ).toBeVisible();
});
...
test('shows "no results" state for unmatched search', async ({ page }) => {
  ...
  await expect(page.getByText(/try different|broaden your search|check spelling/i)).toBeVisible();
