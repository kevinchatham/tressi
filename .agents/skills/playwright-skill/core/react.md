...
module.exports = defineConfig({
  ...
  webServer: {
    command: process.env.CI ? 'npm run build && npx serve -s build -l 5173' : 'npm run dev',
    url: 'http://localhost:5173',
    reuseExistingServer: !process.env.CI,
    timeout: 120_000,
  },
...
test('calls onChange callback when count changes', async ({ mount }) => {
  const values = [];
  const component = await mount(<Counter initialCount={0} onChange={(val) => values.push(val)} />);
