...
describe('FileUtils', () => {
  ...
  describe('ensureDirectoryExists', () => {
    ...
    it('should ignore EEXIST error', async () => {
      ...
      await expect(
        FileUtils.ensureDirectoryExists('/test/dir'),
  ...
  describe('getSafeDirectoryName', () => {
    it('should return _unnamed for empty or invalid input', () => {
      ...
      expect(FileUtils.getSafeDirectoryName(null as unknown as string)).toBe(
        '_unnamed',
      );
    });

    it('should sanitize invalid characters', () => {
      expect(FileUtils.getSafeDirectoryName('folder/subfolder:name')).toBe(
        'folder/subfolder_name',
      );
      expect(FileUtils.getSafeDirectoryName('CON')).toBe('_CON');
      expect(FileUtils.getSafeDirectoryName('  spaced folder  ')).toBe(
        'spaced_folder',
      );
    });
