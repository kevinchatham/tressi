  ...
  constructor() {
    effect(() => {
      ...
      if (bytes === undefined || bytes === null || bytes === 0) {
        this.el.nativeElement.textContent = '0 B';
        return;
      }
      ...
      if (absValue < 1024) {
        this.el.nativeElement.textContent = `${Math.round(bytes)} B`;
      } else if (absValue < 1024 * 1024) {
        this.el.nativeElement.textContent = `${(bytes / 1024).toFixed(1)} KB`;
      } else if (absValue < 1024 * 1024 * 1024) {
        this.el.nativeElement.textContent = `${(bytes / (1024 * 1024)).toFixed(
          1,
        )} MB`;
      } else {
