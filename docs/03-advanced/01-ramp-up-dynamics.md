...
- **Metric Lag**: Because peak RPS is calculated over a sliding window (up to 5 seconds), realtime charts will exhibit a slight lag. The target RPS will not be reflected in the metrics until the sliding window has fully transitioned past the ramp up period.
